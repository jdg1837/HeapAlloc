...
void *professorthread(void *junk) 
{
	...
	while (1) 
	{
		...
		if(helped == 5)
		{
			if(to_help == CLASSA)
			{
				//if there are no students from the other class waiting
				//but there are from the current class
				//we take one more of current class
				if(outside[CLASSB] == 0 && outside[CLASSA] > 0)
				{
					helped -= 1;
				}
				//if that is not the case, we simply start helping 
			...
			//same functionaly, but to switch from class b to class a
			else 
			{
				if(outside[CLASSA] == 0 && outside[CLASSA] > 0)
				{
					helped -= 1;
				}				
				else
		...
		if(to_help == CLASSA)
		{
			...
			//if there are no students needing to be helped for that class
			//we try to help students from the other class
			//ERROR. THIS COULD LEAD TO STARVATION. FIX
			else
			{
				to_help = CLASSB;
				helped = 0;
